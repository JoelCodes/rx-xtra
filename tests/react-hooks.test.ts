...
describe('React Hooks', () => {
  ...
  describe('useSubscribe', () => {
    ...
    it('does not resubscribe on rerender if deps has not changed', () => {
      ...
      const {rerender} = renderHook(({cb, deps}: {cb: () => Subscription; deps: any[]}) => {
    ...
    it('unsubscribes from the observable on unmount', () => {
      ...
      const {unmount} = renderHook(({cb}: {cb: () => Subscription}) => {
        useSubscribe(cb);
      }, {
        initialProps: {cb},
      });
    ...
    it('unsubscribes and resubscribes when the deps change', () => {
      ...
      const unsubscribe = jest.fn();
      const cb = () => NEVER.pipe(tap({subscribe, unsubscribe})).subscribe();
      const {rerender, unmount} = renderHook(({cb, deps}: {cb: () => Subscription; deps: [number]}) => {
        useSubscribe(cb, deps);
      }, {
